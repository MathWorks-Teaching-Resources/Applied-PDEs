function u = ExplicitPDESoln(f,alpha,beta,tEnd,dx,gamma,L)
...
% u^(j+1) = A*u^(j)+b^(j)
A = gallery('tridiag',mu*baseDiag(1:end-1),(1-2*mu)*baseDiag,mu*baseDiag(1:end-1));
...
title("Solving $\frac{\partial u}{\partial t} = \gamma \frac{\partial^2 u}{\partial x^2}$ explicitly","Interpreter","latex")
...
for j = 0:(tEnd/dt - 1)
    ...
    plot(xVals,[alpha((j+1)*dt);u;beta((j+1)*dt)],"SeriesIndex",2,"LineWidth",4)
    subtitle("$t = $"+dt*(j+1))
    drawnow
    pause(0.1)     
    hold off
